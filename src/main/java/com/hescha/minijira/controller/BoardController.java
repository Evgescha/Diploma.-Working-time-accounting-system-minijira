...
@RequestMapping(BoardController.CURRENT_ADDRESS)
public class BoardController {
    public static final String API_PATH = "board";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Board());
        } else {
            model.addAttribute("entity", service.read(id));
        }

        model.addAttribute("project_list", projectService.readAll());
        model.addAttribute("column", columnService.readAll());
    ...
    @PostMapping
    public String save(@ModelAttribute Board entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Board createdEntity = service.create(entity);
